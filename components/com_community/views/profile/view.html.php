...
if (!class_exists('CommunityViewProfile')) {
    ...
    class CommunityViewProfile extends CommunityView {
        ...
        private function _addSubmenu() {
            ...
            $this->addSubmenuItem('index.php?option=com_community&view=profile&task=golfcourse', JText::_('Favorite Courses'));
            ...
            $this->addSubmenuItem('index.php?option=com_community&view=profile&task=blacklist', JText::_('Blacklist'));
                                ...
                                switch ($field['type']) {
                                    ...
                                    case 'text':
                                        if (CValidateHelper::email($field['value'])) {
                                            ...
                                            $profileField[$key][$pKey]['searchLink'] = CRoute::_('index.php?option=com_community&view=search&task=field&' . $field['fieldcode'] . '=' . base64_encode($field['value']));
                                        }
                                        break;
                                    case 'select':
                                    case 'singleselect':
                                        ...
                                        $profileField[$key][$pKey]['searchLink'] = str_replace('{{field_value}}', base64_encode($field['value']), $profileField[$key][$pKey]['searchLink']);
                                    ...
                                    case 'checkbox':
                                        ...
                                        foreach ($checkboxArray as $item) {
                                            if (!empty($item))
                                                $profileField[$key][$pKey]['searchLink'][$item] = CRoute::_('index.php?option=com_community&view=search&task=field&' . $field['fieldcode'] . '=' . base64_encode($item) . '&type=' . $field['type']);
                                        }
                                        break;
                                    case 'country':
                                    ...
                                    case 'location':
                                        if (!empty($field['fieldcode'])) {
                                            ...
                                            $profileField[$key][$pKey]['searchLink'] = CRoute::_('index.php?option=com_community&view=search&task=field&' . $field['fieldcode'] . '=' . base64_encode($location->name));
                                        }
                                        break;
                                    default:
                                        break;
		...
		public function blacklist($data = null) {
			 ...
			 * Opengraph
			 */
			CHeadHelper::setType('website', 'Blacklist Members');
			...
			if($step=="search") {		
				$searchrows = $pModel->getSearchUser($fields);
				$rows = $pModel->getBlacklistMember($my->id);
			} else if($step=="savedata") {		
				if($fields[selmember]=="") {
					$mainframe->enqueueMessage('Please select user.', 'error');
					$searchrows = $pModel->getSearchUser($fields);
				} else {
					$savedata = $pModel->saveBlacklistMember($my->id, $fields);
					$mainframe->enqueueMessage('Added blacklist member.', 'success');
					$fields= array();
				}
				$rows = $pModel->getBlacklistMember($my->id);
			
			} else if($step=="delmem") {	
				$deletedata = $pModel->deleteBlacklistMember($my->id, $fields);
				$mainframe->enqueueMessage('Delete blacklist member.', 'success');
				$rows = $pModel->getBlacklistMember($my->id);
			} else {
				$rows = $pModel->getBlacklistMember($my->id);
			}
		...
		public function golfcourse($data = null) {
			$mainframe = JFactory::getApplication();
			$jinput = $mainframe->input;
			...
			$actstep=$fields[actstep];
			if($step=="search") {		
				$searchrows = $pModel->getSearchCourse($fields);
				$rows = $pModel->getFavoriteGolf($my->id);
				$rowsHome = $pModel->getHomeGolf($my->id);
			} else if($step=="searchHome") {		
				$searchrows = $pModel->getSearchCourse($fields);
				$rows = $pModel->getFavoriteGolf($my->id);
				$rowsHome = $pModel->getHomeGolf($my->id);
			} else if($step=="savedata") {		
				if($fields[selcourse]=="") {
					$mainframe->enqueueMessage('Please select Course.', 'error');
					$searchrows = $pModel->getSearchCourse($fields);
				} else {
					$savedata = $pModel->saveFavoriteGolf($my->id, $fields);
					$mainframe->enqueueMessage('Added Favorite Golf Course.', 'success');
					$fields= array();
				}
				$rows = $pModel->getFavoriteGolf($my->id);
				$rowsHome = $pModel->getHomeGolf($my->id);
				
			} else if($step=="savedataHome") {		
				if($fields[selcourse]=="") {
					$mainframe->enqueueMessage('Please select Course.', 'error');
					$searchrows = $pModel->getSearchCourse($fields);
				} else {
					$savedata = $pModel->saveHomeGolf($my->id, $fields);
					$mainframe->enqueueMessage('Added Home Golf Course.', 'success');
					$fields= array();
				}
				$rows = $pModel->getFavoriteGolf($my->id);
				$rowsHome = $pModel->getHomeGolf($my->id);
				...
				$deletedata = $pModel->deleteFavoriteGolf($my->id, $fields);
				$mainframe->enqueueMessage('Delete Favorite Golf Course.', 'success');
				$rows = $pModel->getFavoriteGolf($my->id);
				$rowsHome = $pModel->getHomeGolf($my->id);
				
			} else if($step=="delcouHome") {	
				$deletedata = $pModel->deleteHomeGolf($my->id, $fields);
				$mainframe->enqueueMessage('Delete Home Golf Course.', 'success');
				$rows = $pModel->getFavoriteGolf($my->id);
				$rowsHome = $pModel->getHomeGolf($my->id);
			...
			echo $tmpl->set('my', $my)
					->set('config', $config)
					->set('rows', $rows)
					->set('rowsHome', $rowsHome)
					->set('searchrows', $searchrows)
					->set('fieldset', $fields)
					->set('step', $step)
					//->set('pagination', $data->pagination)
					->fetch('profile/golfcourse');
		}
