...
class CommunityModelProfile extends JCCModel
{
	...
	public function getBlacklistMember( $userid)
	{
		$db		= $this->getDBO();
		$orderBy	= '';
		$limitSQL = '';
		...
		$orderBy	= ' ORDER BY a.uid ASC ';	

		$query	= 'SELECT a.* FROM '
				. $db->quoteName('#__gwm_member_blacklist') . ' AS a '
				. ' WHERE 1'
				. $extraSQL
				. $orderBy;
	...
	public function getSearchUser($fields)
	{
		$db		= $this->getDBO();
		$orderBy	= '';
		$limitSQL = '';
		...
		$extraSQL = ' AND ( a.name Like ' . $db->Quote('%'.$fields['searchkey'].'%');
		$extraSQL.= ' OR a.username Like ' . $db->Quote('%'.$fields['searchkey'].'%');
		$extraSQL.= ' ) ';
	...
	public function saveBlacklistMember($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		$query	= 'INSERT INTO '. $db->quoteName('#__gwm_member_blacklist')
					.' SET ' . $db->quoteName('MemberID').' = '.$db->Quote($id)
					. ', '. $db->quoteName('BlacklistID').' = '.$db->Quote($fieldset['selmember']);
	...
	public function deleteBlacklistMember($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		$query	= 'DELETE FROM '. $db->quoteName('#__gwm_member_blacklist')
				.' WHERE ' . $db->quoteName('uid').' = '.$db->Quote($fieldset['bid'])
				.' AND ' . $db->quoteName('MemberID').' = '.$db->Quote($id);
	...
	public function getFavoriteGolf( $userid)
	{
		$db		= $this->getDBO();
		$orderBy	= '';
		$limitSQL = '';
	...
	public function getSearchCourse($fields)
	{
		$db		= $this->getDBO();
		$orderBy	= '';
		$limitSQL = '';
		...
		$extraSQL = ' AND ( a.Name Like ' . $db->Quote('%'.$fields['searchkey'].'%');
		$extraSQL.= ' OR a.City Like ' . $db->Quote('%'.$fields['searchkey'].'%');
		$extraSQL.= ' ) ';
	...
	public function saveFavoriteGolf($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		$query	= 'INSERT INTO '. $db->quoteName('#__gwm_member_favorite_courses')
					.' SET ' . $db->quoteName('MemberID').' = '.$db->Quote($id)
					. ', ' . $db->quoteName('CourseID').' = '.$db->Quote($fieldset['selcourse']);
	...
	public function deleteFavoriteGolf($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		$query	= 'DELETE FROM '. $db->quoteName('#__gwm_member_favorite_courses')
				.' WHERE ' . $db->quoteName('uid').' = '.$db->Quote($fieldset['cid'])
				.' AND ' . $db->quoteName('MemberID').' = '.$db->Quote($id);
	...
	public function getHomeGolf( $userid)
	{
            $db		= $this->getDBO();
            $orderBy	= '';
            $limitSQL = '';
            ...
            $query	= 'SELECT a.* FROM '
                            . $db->quoteName('#__gwm_member_home_courses') . ' AS a '
                            . ' WHERE 1'
                            . $extraSQL
                            . $orderBy;
	...
	public function saveHomeGolf($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		$query	= 'INSERT INTO '. $db->quoteName('#__gwm_member_home_courses')
					.' SET ' . $db->quoteName('MemberID').' = '.$db->Quote($id)
					. ', ' . $db->quoteName('CourseID').' = '.$db->Quote($fieldset['selcourse']);
	...
	public function deleteHomeGolf($id, $fieldset)
    {
        $my = CFactory::getUser();
        $db= $this->getDBO();
		...
		$query	= 'DELETE FROM '. $db->quoteName('#__gwm_member_home_courses')
				.' WHERE ' . $db->quoteName('uid').' = '.$db->Quote($fieldset['cid'])
				.' AND ' . $db->quoteName('MemberID').' = '.$db->Quote($id);
	...
	public function saveProfile($userId, $fields)
	{
        ...
        public function getUserLatLon($userid)
        {
